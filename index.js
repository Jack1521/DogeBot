...
multi = true
harga = 0
matauang = 'USD'Kiyopref = true
numbernye = '0'
autoketik = false;
prefixStatus = false;
targetpc = "989035024059"; 
owner = "989035024059"; 
fakeyoi = "Jack"; 
HunterApi = "Ikyy69", 
xchillds = 'XChillDs' 
...
const checkRegisteredUser = (sender) => {
    ...
    const ownerNumber = [`989035024059@s.whatsapp.net`]
   ...
   const isRegister = register.includes(sender)
    ...
    const totalhit = JSON.parse(fs.readFileSync("./lib/totalcmd.json"))[0]
      .totalcmd;
const daftar1 = `Hi ${pushname} ${ucapanWaktu} \n\nPlease verify first Oniisan^^ `
       const daftar2 = '```Click the button below to verify bro or type Jack The Ripper\n𝐉𝐚𝐜𝐤 𝐁𝐨𝐭```'
       const daftar3 = [
          {
            buttonId: `verify`,
            buttonText: {
...
/*const reply = (teks) => {
            xeon.sendMessage(from, teks, text, { thumbnail: iye, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jam} - ${week} ${weton} - ${date}`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/989035024059?text=hello`}}})
        }*/
...
xeon.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
    ...
    const add = function (from, orangnya) {
      xeon.groupAdd(from, orangnya);
        ...
        const grupinv = (teks) => {
            grup = xeon.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://wa.link/zmazip', "groupName": `Jack Inc.`, "footerText": "*_𝐉𝐚𝐜𝐤 𝐁𝐨𝐭_*", "jpegThumbnail": iye, "caption": teks}}, {quoted:mek})
...
const fakeitem = (teks) => {
        ...
        participant:`0@s.whatsapp.net`, ...(from ? {
remoteJid :"6289523258649-1604595598@g.us" }: {})
                    },message:{"orderMessage":{"orderId":"174238614569481","thumbnail":fs.readFileSync(`./stik/thumb.jpeg`),"itemCount":10,"status":"INQUIRY","surface":"CATALOG","message":`𝐉𝐚𝐜𝐤 𝐁𝐨𝐭`,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}}, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true})}
// Katalog
const reply = (teks) => {
...
const fakewa = (teks) => {        
xeon.sendMessage(from, teks, text, { thumbnail: fake, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `Xeon`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/989035024059?text=hello`}}})
        }
...
sendEphemeral: false,
"externalAdReply": {
                "title": `Whatsapp-BOT\n𝐉𝐚𝐜𝐤 𝐁𝐨𝐭. (@nnda.krnwn_) •Instagram photos and Videos`,
                "body": "",
                "previewType": "PHOTO",
...
"externalAdReply": {
                "title": `Whatsapp`,
                "body": `wa.me/989035024059`,
                "mediaType": "10",
                "mediaUrl": `wa.me/989035024059`,
                "thumbnailUrl": "https://i.ibb.co/FbMnwYF/logo.jpg",
                "thumbnail": iye,
                "sourceUrl": `wa.me/989035024059`,
...
async function sendFileFromUrl(from, url, caption, msg, men) {
    ...
    if (isGroup && !isVote) {
      if (budy.toLowerCase() === "vote") {
        ...
        let id_vote = sender ? sender : "989035024059@s.whatsapp.net";
...
*「 𝐉𝐚𝐜𝐤 𝐁𝐨𝐭 」*`
         haitod = await getBuffer(`http://hadi-api.herokuapp.com/api/card/verify?nama=${encodeURI(pushname)}&member=${_registered.length}&seri=${serialUser}&pp=${ppimg}&bg=${ppimg}`)
             buttons = [{buttonId: `menu`,buttonText:{displayText: `</️MENU`},type:1}]
              ...
              buttonsMessage = {footerText:'By Jack', imageMessage: imageMsg,
        ...
        // console.log(e)
            ...
            reply(`*Thank you for being registered 𝐉𝐚𝐜𝐤 𝐁𝐨𝐭*`)
        }, 2000)
        break
...
groups = xeon.chats.array.filter(v => v.jid.endsWith('g.us'))
              ...
              total = math(`${groups.length}*${privat.length}`)
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ptod = "989035024059@s.whatsapp.net"
        dtod = '989035024059'
        stod = `${sender}`
        uwuu = '```'
        yoii = '*'
...
case 'bugreport':
              ...
              xeon.sendMessage('989035024059@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
...
㋛${prefix}𝙩𝙬𝙞𝙩𝙩𝙚𝙧 <𝙪𝙧𝙡>
`
sendButMessage(from, menu, `𝐉𝐚𝐜𝐤 𝐁𝐨𝐭`, [
          {
            buttonId: `command`,
            buttonText: {
     ...
     break
        ...
        case "broadcast":  
                   if (!isOwner && !mek.key.fromMe) return reply('only owner can use this feature')    
                    ...
                    buff = await xeon.downloadMediaMessage(encmedia)
                    for (let _ of anu) {
                        xeon.sendMessage(_.jid, buff, image, {quoted:fakestatus ,caption: `「  *𝐉𝐚𝐜𝐤 𝐁𝐨𝐭 BROADCAST* 」\n\n${ini_bc}`})
                    }
                    reply('```Success Broadcast```')
                    } else {
                    for (let _ of anu) {
                    xeon.sendMessage(_.jid, `*𝐉𝐚𝐜𝐤 𝐁𝐨𝐭 BROADCAST*\n${ini_bc}`, text, { contextInfo: { mentionedJid: [sender],"forwardingScore":999,"isForwarded":true},sendEphemeral: true })
                    }
                    reply('```Success Broadcast```')
                    }
                    break
...
buttons = [{buttonId: `writelist`,buttonText:{displayText: `Write List🔖`},type:1}]
              ...
              buttonsMessage = {footerText:'𝐉𝐚𝐜𝐤 𝐁𝐨𝐭', imageMessage: imageMsg,
...
sendButMessage(from, tiyo, `𝐉𝐚𝐜𝐤 𝐁𝐨𝐭`, [
            {              
   ...
   case 'bc':
      case 'broadcast':
        if (!mek.key.fromMe) return;
             ...
             for (let _ of anu) {
             xeon.sendMessage(_.jid, bc, image, {quoted:fkontak,caption: `*「 𝐉𝐚𝐜𝐤 𝐁𝐨𝐭 BROADCASTING 」*\n\n${body.slice(4)}`})
}
